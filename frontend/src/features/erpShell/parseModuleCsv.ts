...
function buildArapTx(mode: 'AP' | 'AR', row: Record<string, string>): Record<string, unknown> | null {
  ...
  return {
    ...
    company_currency: cell(row, 'company_currency'),
    company_amount: parseNum(cell(row, 'company_amount')),
    company_tax_amount: parseNum(cell(row, 'company_tax_amount')),
    exchange_rate: parseNum(cell(row, 'exchange_rate')),
